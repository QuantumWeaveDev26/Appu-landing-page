...
**Tech Stack:** TypeScript, Fastify, PostgreSQL, Zod, Node.js crypto (HMAC-SHA256), Vitest (`npm test`).  
...
- [ ] **Step 1.3: Unit Test Repository Operations**
  Create `backend/src/domain/whatsapp/proactive/repository.test.ts`:
...
- [ ] **Step 2.4: Unit Tests for Content Generation**
  Create `backend/src/domain/whatsapp/proactive/generators.test.ts`:
  ...
  - Run `npm test -- generators.test.ts`.
...
- [ ] **Step 3.3: Integration Tests**
  Create `backend/test/whatsapp-proactive-routes.test.ts`:
  ...
  - Run `npm test -- whatsapp-proactive-routes.test.ts`.
...
3. **Route Integration Tests:**
   `npm test -- whatsapp-proactive-routes.test.ts`
   Verifies HMAC authentication enforcement, replay prevention, and HTTP response contracts.
4. **Full Backend Test Suite:**
   `npm test` in `backend/` to verify zero regressions across existing 12+ test suites.
