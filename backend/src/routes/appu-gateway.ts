...
export const appuGatewayRoutes: FastifyPluginAsync<AppuGatewayRouteOptions> = async (
   ...
   * server-side entitlement derivation, and child personalisation context injection.
   */
  fastify.post('/api/appu/message', { preHandler: requireAuth }, async (request, reply) => {
    ...
    // 4. Build safe server-owned child AI context
    ...
    // 6. Forward to n8n via secure client
    let response: any;
    try {
    ...
    // NOTE for Future Billing Accounting:
    // Session usage incrementation hook (e.g. usage_records for monthly_ai_sessions / voice_minutes)
    // will be recorded here after successful n8n response.
