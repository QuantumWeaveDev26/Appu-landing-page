...
describe('WhatsApp Conversational Onboarding Backend', () => {
  ...
  describe('Link-Later Web Account Merging', () => {
    test('authenticated web user claims phone-only household via POST /api/household/onboard', async () => {
      const phone = '+919123456789';

      // 1. User onboards partially on WhatsApp
      await WhatsAppOnboardingService.saveStep(db, {
        phone,
        fields: {
          name: 'Siddharth',
          grade: 'Grade 7',
      ...
      const phoneHousehold = await WhatsAppOnboardingRepository.findHouseholdByPhone(db, phone);
      assert.ok(phoneHousehold);

      // Verify it is a phone-only household (zero household_members)
      const membersBefore = await TenancyRepository.getHouseholdMembers(db, phoneHousehold.id);
      assert.equal(membersBefore.length, 0);

      // 2. Parent signs up on web and calls /api/household/onboard with phone
      const parentUserId = crypto.randomUUID();
      const webRes = await app.inject({
        method: 'POST',
        url: '/api/household/onboard',
        headers: {
          authorization: `Bearer ${parentUserId}`
        },
        payload: {
          householdName: 'Siddharth Family',
          phone
        }
      });

      assert.equal(webRes.statusCode, 200);
      const webData = webRes.json();
      assert.equal(webData.household.id, phoneHousehold.id);
      assert.equal(webData.role, 'OWNER');
      assert.equal(webData.isNew, false);

      // Verify household_members now contains the web user
      const membersAfter = await TenancyRepository.getHouseholdMembers(db, phoneHousehold.id);
      assert.equal(membersAfter.length, 1);
      assert.equal(membersAfter[0].userId, parentUserId);
      assert.equal(membersAfter[0].role, 'OWNER');

      // Verify child profile is retained
      const children = await TenancyRepository.listChildProfilesByHousehold(db, phoneHousehold.id);
      assert.equal(children.length, 1);
      assert.equal(children[0].preferredName, 'Siddharth');
    });

    test('does not claim already-owned household on phone collision', async () => {
      const phone = '+919555444333';

      // 1. Initial phone-only household created and claimed by first parent
      await WhatsAppOnboardingService.saveStep(db, {
        phone,
        field: 'name',
        value: 'Aanya'
      ...
      const firstClaim = await TenancyService.onboardParentHousehold(db, {
        userId: firstUserId,
        phone
      });
      assert.equal(firstClaim.isNew, false);

      // 2. Second parent tries to onboard with the same phone
      const secondUserId = crypto.randomUUID();
      const secondClaim = await TenancyService.onboardParentHousehold(db, {
        userId: secondUserId,
        phone
      });

      // Second user gets a new, isolated household rather than stealing the first owner's household
      assert.equal(secondClaim.isNew, true);
      assert.notEqual(secondClaim.household.id, firstClaim.household.id);
    });
  });
});
